...
var smskmaxabs = tryRequire( resolve( __dirname, './../lib/ndarray.native.js' ) );
var opts = {
	'skip': ( smskmaxabs instanceof Error )
};
...
tape( 'main export is a function', opts, function test( t ) {
	...
	t.strictEqual( typeof smskmaxabs, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 7', opts, function test( t ) {
	t.strictEqual( smskmaxabs.length, 7, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the maximum absolute value of a strided array according to a mask', opts, function test( t ) {
	var mask;
	var x;
	var v;
	...
	v = smskmaxabs( x.length, x, 1, 0, mask, 1, 0 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', opts, function test( t ) {
	...
	v = smskmaxabs( 0, x, 1, 0, mask, 1, 0 );
	t.strictEqual( isnanf( v ), true, 'returns expected value' );

	v = smskmaxabs( -1, x, 1, 0, mask, 1, 0 );
	t.strictEqual( isnanf( v ), true, 'returns expected value' );
